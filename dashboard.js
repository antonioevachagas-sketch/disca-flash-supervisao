...
document.querySelectorAll('[data-open]').forEach(n=>n.onclick=()=>{if(n.dataset.open==='uploadDialog'){if(!allocationDraft.size){const people=activeAllocationPeople();if(selectedConsultant){el('allocationTarget').value='single';allocationDraft.set(selectedConsultant,{selected:true,count:0})}else people.forEach(p=>allocationDraft.set(p.id,{selected:true,count:0}));saveUploadDraft()}renderAllocations();}el(n.dataset.open).showModal();document.body.classList.add('modal-open')});
...
const allocationDraft=new Map(),analysisCache=new Map();let uploadAnalysis=null,analysisSequence=0,uploadRequest=null,uploadPhase='idle',autoCampaignName='',lastUploadSuccess='';
...
function setUploadProgress(active,title='',message=''){
 ...
 el('analyzeUpload').textContent=uploadPhase==='analyzing'?'Analisando planilha…':'Analisar novamente';
}
...
function restoreUploadDraft(){
 try{const draft=JSON.parse(sessionStorage.getItem(uploadDraftKey)||'null');if(!draft)return;
  ...
  el('uploadDraftStatus').textContent=draft.fileName?'Rascunho recuperado · selecione '+draft.fileName+' novamente':'Rascunho recuperado';if(draft.fileName)setFileQuickStatus('Selecione '+draft.fileName+' novamente para continuar.');
 }catch{sessionStorage.removeItem(uploadDraftKey)}
}
function clearUploadDraft(){
 analysisSequence++;uploadAnalysis=null;uploadRequest=null;uploadPhase='idle';autoCampaignName='';lastUploadSuccess='';allocationDraft.clear();sessionStorage.removeItem(uploadDraftKey);el('uploadForm').reset();el('analyzeUpload').classList.add('hidden');el('uploadMsg').textContent='';el('uploadMsg').style.color='';el('uploadDraftStatus').textContent='Novo rascunho';setFileQuickStatus('Nenhum arquivo selecionado');updateAdvancedSummary();setUploadProgress(false);renderAllocations();
}
...
function updateDistributionSummary(){
 ...
 if(lastUploadSuccess&&!a)message=lastUploadSuccess;
 ...
 const readyTitle=uploadPhase==='analyzing'?'Analisando planilha':lastUploadSuccess&&!a?'Distribuição concluída':valid?'Pronto para distribuir':a?'Revise a divisão':'Aguardando planilha';
 const readySummary=valid?number(available)+' contatos · '+list.length+' consultor(es)':uploadPhase==='analyzing'?'Verificando contatos válidos e repetidos':lastUploadSuccess&&!a?'Contatos enviados com segurança':a?'Confira as orientações acima':'Selecione um arquivo para começar';
 const steps=el('uploadForm').querySelectorAll('.upload-steps li'),hasFile=!!el('masterFile').files?.[0];steps.forEach((step,i)=>{step.classList.toggle('done',i===0?!!a:i===1?valid:false);step.classList.toggle('current',i===0?hasFile&&!a:i===1?!!a&&!valid:i===2&&valid)});
 el('distributionSummary').textContent=message;el('distributionSummary').classList.toggle('ready',valid||!!lastUploadSuccess);el('uploadReadyTitle').textContent=readyTitle;el('uploadReadySummary').textContent=readySummary;el('distributeSubmit').textContent=single&&chosen?'Enviar somente para '+chosen.display_name:'Confirmar distribuição';el('distributeSubmit').disabled=!valid||mutationPending||uploadPhase!=='idle';return valid;
}
...
el('masterFile').onchange=()=>{analysisSequence++;uploadAnalysis=null;uploadRequest=null;lastUploadSuccess='';el('redistributeUnworked').checked=false;el('uploadMsg').textContent='';const file=el('masterFile').files[0];el('analyzeUpload').classList.toggle('hidden',!file);if(file){const suggested=campaignFromFile(file);if(!el('campaignName').value.trim()||el('campaignName').value===autoCampaignName)el('campaignName').value=suggested;autoCampaignName=suggested;setFileQuickStatus(file.name+' · '+fileSize(file.size)+' · aguardando análise')}else setFileQuickStatus('Nenhum arquivo selecionado');saveUploadDraft();renderAllocations();if(file)analyzeSelectedFile()};
el('analyzeUpload').onclick=()=>analyzeSelectedFile(true);
for(const event of ['dragenter','dragover'])el('uploadDropZone').addEventListener(event,e=>{e.preventDefault();el('uploadDropZone').classList.add('dragging')});
for(const event of ['dragleave','drop'])el('uploadDropZone').addEventListener(event,e=>{e.preventDefault();el('uploadDropZone').classList.remove('dragging')});
el('uploadDropZone').addEventListener('drop',e=>{const file=e.dataTransfer?.files?.[0];if(!file)return;try{const transfer=new DataTransfer();transfer.items.add(file);el('masterFile').files=transfer.files;el('masterFile').dispatchEvent(new Event('change',{bubbles:true}))}catch{setFileQuickStatus('Use o botão Escolher arquivo neste navegador.','error')}});
el('uploadForm').addEventListener('input',e=>{if(e.target.id!=='masterFile')saveUploadDraft();updateAdvancedSummary()});
...
el('uploadForm').onsubmit=async e=>{
 ...
 try{const result=await api('upload',fd,true);done=true;const report=result.distributions?.map(d=>d.consultant_name+': '+number(d.imported)).join(' · '),only=result.distributions?.[0],success=single&&only?'Planilha enviada somente para '+only.consultant_name+': '+number(only.imported)+' contatos.':result.message+(report?' '+report:'');lastUploadSuccess=success+(result.unassigned?' '+number(result.unassigned)+' contatos ficaram sem distribuição.':'');el('uploadMsg').style.color='var(--mint)';el('uploadMsg').textContent=lastUploadSuccess;form.reset();sessionStorage.removeItem(uploadDraftKey);analysisCache.clear();uploadAnalysis=null;uploadRequest=null;autoCampaignName='';allocationDraft.clear();el('analyzeUpload').classList.add('hidden');el('redistributeUnworked').checked=false;setFileQuickStatus('Distribuição concluída com segurança','success');el('uploadDraftStatus').textContent='Distribuição concluída';selectedConsultant='';selectedCampaign='';leadOffset=0;await refresh(false,true);showView('campaigns');toast(single?'Planilha enviada para um único consultor.':'Distribuição concluída. Confira o resultado no formulário.');}
