...
async function api(action,data={},form=false,retry=true){
  ...
  if(!res.ok){if(res.status===401&&action!=='login')logout();throw Object.assign(new Error(body.error||'Não foi possível concluir a operação.'),{status:res.status})}return body;
}
