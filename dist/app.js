...
async function api(action,data={},form=false,retry=true){
  ...
  if(!res.ok){if(res.status===401&&action!=='login')logout();throw Object.assign(new Error(body.error||'Não foi possível concluir a operação.'),{status:res.status})}if(!['overview','preview_upload','login','refresh','health'].includes(action))clearOverviewCache();return body;
}
...
async function refresh(show=false,rethrow=false,allowCached=false){
 ...
 if(!overview)el('syncStatus').textContent='Carregando dados da equipe…';else if(!cached||!allowCached)el('syncStatus').textContent='Atualizando dados…';
 try{
  let pending=overviewRequests.get(key);
  if(!pending){pending=api('overview',params);overviewRequests.set(key,pending);const cleanup=()=>{if(overviewRequests.get(key)===pending)overviewRequests.delete(key)};pending.then(cleanup,cleanup)}
  const result=await pending;if(generation===overviewGeneration&&token)rememberOverview(key,result);
  if(seq!==refreshSequence||generation!==overviewGeneration||!token)return result;
  apply(result);el('syncStatus').textContent='Atualizado às '+shortTimeFormat.format(new Date());el('syncStatus').parentElement.classList.remove('error');if(show)toast('Dados atualizados');return result;
...
async function loadConsultant(id){selectedConsultant=id;selectedCampaign='';leadOffset=0;await refresh()}async function loadLeads(id){selectedCampaign=id;leadOffset=0;await refresh()}async function changeLeadPage(offset){leadOffset=Math.max(0,Number(offset)||0);await refresh(false,false,true)}
